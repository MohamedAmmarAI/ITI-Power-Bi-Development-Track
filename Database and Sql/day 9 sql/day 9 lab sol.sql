...
create  proc num_studs_departments
as
    select d.dept_name,count(*) as numbers_of_student
	from Department d join sales.student s
	on d.Dept_Id=s.Dept_Id
	group by d.Dept_Name

num_studs_departments




-- 2.	Create a stored procedure that will check for the # of employees in the project p1 
--      if they are more than 3 print message to the user “'The number 
--      of employees in the project p1 is 3 or more'” if they are less display a 
--      message to the user “'The following employees work for the project p1'” in addition to the first name and last name of each one. [Company DB] 
...
as 
  begin
	 ...
	 on p.Pnumber=w.Pno join Employee e
	 on e.SSN=w.ESSn
	 where p.Pname='p1'
...
create proc replace_employee @old_emp_no int , @new_emp_no int , @project_num int
as
  begin
      update works_on
	  set ESSN=@new_emp_no
	  where ESSN=@old_emp_no and PNO=@project_num
  end
       
exec replace_employee 2,3,100
...
INSERT INTO audit_table (ProjectNo, UserName, ModifiedDate, Budget_Old, Budget_New)
